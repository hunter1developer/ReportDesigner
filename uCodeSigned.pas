...
// WinVerifyTrust, so it NT only.  IsCompanySigningCertificate works on Win9x,
...
type
  ...
  TCompanySignedCertificate = class
  private
    FCompanySigningCertificate: boolean;
    function IsCompanySigningCertificate(const Filename, CompanyName :string): Boolean;
  public
    constructor Create(const fileName: string; const companyName: string);
    property CompanySigningCertificate: boolean read FCompanySigningCertificate;
  end;
  ...
  public
    ...
    property CodeSigned: boolean read FCodeSigned;
  end;
...
constructor TCompanySignedCertificate.Create(const fileName, companyName: string);
begin
  FCompanySigningCertificate := IsCompanySigningCertificate(fileName, companyName);
end;

constructor TCodeSigned.Create(const fileName: string);
begin
  FCodeSigned := IsCodeSigned(fileName);
end;

function TCodeSigned.IsCodeSigned(const Filename: string): Boolean;
var
...
function TCompanySignedCertificate.IsCompanySigningCertificate(const Filename, CompanyName :string): Boolean;
var
  hExe: HMODULE;
  ...
  try
    ...
    try
      ...
      if not CryptVerifyMessageSignature(VerifyParams, 0, @Cert.bCertificate,
        ...
        Exit;
      try
        // Extract and compare the certificate subject names.  Don't
